...
export default function StudioPage() {
  ...
  /* ---- 同步至官方App（同步码 Sheet，不再直接改 works） ---- */
  const syncCode = makeSyncCode(title.trim() || '未命名设计', params);
  const openSync = () => {
    if (isSynced && savedId.current != null) { toast('该设计已是最新同步，可继续编辑后再次同步'); return; }
    setSyncOpen(true);
  };
  const importDone = () => {
    const t = title.trim() || '未命名设计';
    ...
    setSyncedFlag(true);
    setSyncOpen(false);
    toast('已确认导入 · 作品已标记「已同步」', 'check');
  };

  /* ---- 从作品列表进入：/design/studio?work=id 载入该参数稿 ---- */
  const worksRef = useRef(works);
  worksRef.current = works;
  useEffect(() => {
    const q = new URLSearchParams(location.search);
    const wid = Number(q.get('work') || '');
    if (!wid) return;
    const w = worksRef.current.find((x) => x.id === wid);
    if (!w) return;
    setHs({ params: { ...w.params }, undo: [], redo: [] });
    setTitle(w.title);
    savedSnap.current = { title: w.title, params: { ...w.params } };
    savedId.current = w.id;
    toast(`已载入「${w.title}」到工作台`, 'check');
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  ...
  return (
    <div style={{ height: '100dvh', display: 'flex', flexDirection: 'column', background: '#F7F4F8', overflow: 'hidden' }}>
      ...
      <header style={{
        ...
        <button onClick={() => navigate('/design/sim3d')} aria-label="返回3D模拟中心" title="返回3D模拟中心"
        ...
        <button onClick={openTitleSheet} title="点击修改标题" style={{ flex: 1, minWidth: 0, padding: '2px 6px' }}>
          <div style={{ textAlign: 'center', fontSize: 9, fontWeight: 700, color: 'var(--text-3)', letterSpacing: 1.5, marginBottom: 1 }}>3D模拟 · 参数化建模</div>
        ...
        <div className="row" style={{ gap: 4, flexShrink: 0 }}>
          ...
          <button onClick={openSync} className="row" title="同步至官方App"
            style={{
