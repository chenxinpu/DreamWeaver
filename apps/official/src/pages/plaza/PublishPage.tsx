        ...
        <div className="card" style={{ padding: 14, marginBottom: 12 }}>
          ...
          <div className="col" style={{ gap: 8 }}>
            ...
            {myWorks.length + designLinked.length === 0 && (
              <button className="row" style={{ gap: 8, padding: 12, borderRadius: 12, border: '1.5px dashed var(--brand)', color: 'var(--brand-deep)', fontSize: 13, justifyContent: 'center', background: 'var(--brand-soft)' }} onClick={() => { window.open('http://localhost:5174/#/design/canvas','_blank'); }}>
                <Icon name="pen-tool" size={16} />去设计师App画布起稿后再来关联
              </button>
            )}
          </div>
        </div>
