...
本仓库主干即 **V2 全栈迭代**（依据 `apps/v2_0.md` 实现）：官方端呈现 3D 图与打版图，创作者平台素材库支持导入主流服装 CAD（CLO 3D / Gerber / Lectra 等）导出的 **DXF / OBJ / SVG** 等成果文件；商城与创作者平台都是独立网页形态（参考抖音），商城入口在侧边栏而非底部 Tab；「我的」参考抖音个人主页；已删除品牌孵化与投票打榜。
...
- 创作者中心桌面版（`/creator`，12 模块，BI 规范，保留用于后续打包原生桌面应用）：总览（无资源池引擎/最近订单）/素材库（导入）/作品（品类联动标签、按品类分组管理）/发推文（我的推文 近1/3/7 天进度）/资源池/橱窗材料（无价格字段、被拒可删）/商品管理/数据看板/佣金/通知/设置/审核。
